<?php
    ...
    if (($local_md5sig === $md5sig) && ($status_code == 2)) {
        $check = $mysqli->prepare("SELECT t_id FROM test WHERE order_id = ? LIMIT 1");    
    ...
    if ($order_id) {
        ...
        if (!$mysqli_conn->connect_errno) {
            $stmt = $mysqli_conn->prepare("SELECT * FROM test WHERE order_id = ? LIMIT 1");
            if ($stmt) {
    ...
    function buildReceiptHtml(array $orderData, string $amount): string
    {
        ...
        $html .= '<div class="header"><img src="logo.png" style="height:60px;"><br>YOUR_ORGANIZATION_NAME</div><div class="title">Payment Confirmation</div></div>';
    ...
    if ($order_id && $customer_email) {
        ...
        if ($dbAvailable) {
            ...
            if ($gotLock) {
                // check existing flag
                $stmt = $mysqli->prepare("SELECT COUNT(*) AS c FROM test WHERE order_id = ? AND email_flag IS NOT NULL");
                if ($stmt) {
                    $stmt->bind_param('s', $order_id);
                    $stmt->execute();
                    $result = $stmt->get_result();
            ...
            try {
                ...
                try {
                    ...
                    if (empty($orderData) && $order_id && $dbAvailable) {
                        $stmt = $mysqli->prepare("SELECT * FROM test WHERE order_id = ? LIMIT 1");
                        if ($stmt) {
                            $stmt->bind_param('s', $order_id);
                            $stmt->execute();
                            $result = $stmt->get_result();
                ...
                if ($dbAvailable) {
                    $stmt = $mysqli->prepare("UPDATE test SET email_flag = 1 WHERE order_id = ?");
                    if ($stmt) {
                        $stmt->bind_param('s', $order_id);
                        $stmt->execute();
                        $stmt->close();
                        unset($_SESSION['orders'][$order_id]);
                    } else {
    ...
    <body class="status-page">
        <main class="status-card">
            ...
            <div class="status-actions">
                <a class="status-button" href="index.php">Back to Home</a>&ensp;
                <?php 
                    if ($order_id): ?>
